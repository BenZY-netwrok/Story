    ...
    <template v-slot:header>
        ...
        <div class="flex justify-between items-center">
            <h1 class="text-3xl font-bold text-gray-900">
                {{ route.params.id? model.title : "Create a Post" }}  {{route.params.create}}
            </h1>
        ...
        <form v-else @submit.prevent="savePost(model.owner_user_id)" class="animate-fade-in-down">
            <div class="shadow sm:rounded-md sm:overflow-hidden">
                ...
                <div>
                    ...
                    <button
                        v-if="store.state.currentPost.data.current_user_id === model.owner_user_id || route.params.create" 
                        type="button"
                ...
                <div>
                    ...
                    <input
                    type="text"
                    :disabled="store.state.currentPost.data.current_user_id !== model.owner_user_id && !route.params.create"
                    name="title"
                    id="title"
                    v-model="model.title"
                ...
                <!-- Status -->
                <div class="flex items-start" v-if="store.state.currentPost.data.current_user_id === model.owner_user_id || route.params.create">
                ...
                <div v-if="model.floors && route.params.id" class="px-4 py-5 bg-white space-y-6 sm:p-6">
                    ...
                    <div v-if="!model.floors.length" class="text-center text-gray-600">
                ...
                <div class="px-4 py-3 bg-gray-50 text-right sm:px-6" v-if="store.state.currentPost.data.current_user_id === model.owner_user_id || route.params.create || replyAdding">
                <button
                    type="submit"
...
import FloorEditor from '../components/editor/FloorEditor.vue';
...
const replyAdding = computed(() => store.state.currentPost.replyAdd)

const flors = computed(() => store.state.flors);
...
if(route.params.id) {
    ...
    // model.value = store.state.flors.find(
...
function addReply(index) {
    const newReply = {
        id: uuidv4(),
        owner_user_id: store.state.currentPost.data.current_user_id,
        content: null,
        floorNum: 1
    };
    
     store.dispatch("getReplyAdd")

    model.value.floors.splice(index, 0, newReply);
...
function deleteReply(reply) {
    store.dispatch("getReplyAdd")
    model.value.floors = model.value.floors.filter((r) => r !== reply);
}

function floorChange(reply) {
    model.value.floors = model.value.floors.map((r) => {
        if(r.id === reply.id) {
